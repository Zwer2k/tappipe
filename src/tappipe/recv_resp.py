...
class recv_resp:
    ...
    def __init__(self, parent=None, bytes=[]):
        ...
        if len(self.bytes) < 2:
            _LOGGER.debug("Recv Resp: Not enough bytes")
            return
        ...
        # Packet
        if status & 0x10 == 0:
            (self.decoded["packet_number"],) = struct.unpack(
                ">H", self.bytes[position : position + 2]
            )
            position += 2
        else:
            ...
            position += 1
        # Slot Counter
        (self.decoded["slot_number"],) = struct.unpack(
            ">H", self.bytes[position : position + 2]
        )
        position += 2
        ...
        _LOGGER.info("%d packets of %d length", packets, packetLength)

        for i in range(packets):
            if position >= len(self.bytes):
                return
            if self.bytes[position] == pvtype.POWER_REPORT.value:
                self.decoded["packets"].append(
                    power_report(self, self.bytes[position : position + packetLength])
                )
            if self.bytes[position] == pvtype.TOPOLOGY_REPORT.value:
                self.decoded["packets"].append(
                    topology_report(
                        self, self.bytes[position : position + packetLength]
                    )
                )
